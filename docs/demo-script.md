...
**Live:** [seansabado.github.io/laundromatai-showcase](https://seansabado.github.io/laundromatai-showcase/)
...
3. Create a fake order by clicking **Create Fake Order** in the POS module
4. Switch to `Tenant B` using the switcher

**What to observe:**

- All orders, machines, and queue state **reset completely** on tenant switch
...
3. Click **Create Fake Order** 2–3 times
4. Observe orders appear with status `queued`
5. Observe the **Pending Actions** counter increment in the Offline Queue panel

**What to observe:**

- Orders are NOT dropped when offline — they are enqueued
...
2. Create 2–3 orders while online (they appear with status `created`)
...
- "Every ADR explains a tradeoff. I write them because I want to be able to justify decisions in code review, not just implement them."
