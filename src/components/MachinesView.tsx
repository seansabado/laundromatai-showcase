...
const MACHINE_INFO: Record<string, { type: string; capacity: string }> = {
  "WM-01": { type: "Front-load Washer", capacity: "8 kg" },
  "WM-02": { type: "Front-load Washer", capacity: "8 kg" },
  "DR-01": { type: "Gas Dryer", capacity: "10 kg" },
  "DR-02": { type: "Gas Dryer", capacity: "10 kg" },
};
...
export const MachinesView = () => {
  ...
  const handleState = (machineId: string, state: "idle" | "in_use" | "maintenance") => {
    setMachineState(machineId, state);
    setLastAction(`${machineId} → ${state.replace("_", " ")} at ${new Date().toLocaleTimeString()}`);
  };
    ...
    <div className="dashboard-wrap">
      ...
      <div className="machine-summary-bar">
        <div className="machine-stat">
          <Cpu size={14} />
          <span>Total: <strong>{machines.length}</strong></span>
        </div>
        <div className="machine-stat machine-stat-inuse">
          <Play size={13} />
          <span>In Use: <strong>{inUseCount}</strong></span>
        </div>
      ...
      <div className="machines-card-grid">
        {machines.map((machine) => {
          ...
          return (
            <article key={machine.machineId} className={stateClass[machine.state] ?? "machine-tile"}>
              ...
              <div className="muted-row">Updated: {formatDate(machine.updatedAt)}</div>
      ...
      <section className="panel" style={{ marginTop: "12px" }}>
        ...
        <p className="module-subtitle" style={{ marginTop: "6px" }}>
          Machine state transitions are managed via a local state hook that mirrors the
          Firestore-backed <code>useMachineState</code> hook used in production. In the live app,
          state changes are written to <code>/tenants/{"{tenantId}"}/machines/{"{machineId}"}</code>
          and broadcast to all connected sessions in real time via <code>onSnapshot</code>.
        </p>
      </section>
    </div>
  );
