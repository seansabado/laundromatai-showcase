...
const NAV_ITEMS: { id: View; label: string; Icon: React.ComponentType<{ size?: number }>; gate?: string }[] = [
  { id: "dashboard", label: "Dashboard", Icon: LayoutDashboard },
  { id: "pos", label: "POS Module", Icon: ShoppingCart },
  { id: "machines", label: "Machine Fleet", Icon: Cpu },
  { id: "analytics", label: "Analytics", Icon: BarChart3, gate: "Growth+" },
    ...
    <div className="admin-shell">
      ...
      <aside className="sidebar">
        <div className="sidebar-logo">
          <span className="sidebar-wordmark">LaundromatAI</span>
          ...
          <div className="admin-header-right">
            ...
            <div className="header-dropdown-wrap">
              <button
                ...
                onClick={() => { setTenantDropOpen(!tenantDropOpen); setBranchDropOpen(false); }}
              >
                <span>{currentTenant?.name}</span>
                <span className={`plan-chip plan-chip-${currentTenant?.plan}`}>{currentTenant?.plan}</span>
                <ChevronDown size={13} />
              </button>
              {tenantDropOpen && (
                <div className="header-dropdown-menu">
                  {tenants.map((t) => (
                    <button
                      ...
                      onClick={() => { switchTenant(t.id); setTenantDropOpen(false); }}
                    >
                      {t.name}
                      <span className={`plan-chip plan-chip-${t.plan}`}>{t.plan}</span>
                    </button>
                  ))}
                </div>
              )}
            ...
            <div className="header-dropdown-wrap">
              <button
                className="header-dropdown-btn"
                onClick={() => { setBranchDropOpen(!branchDropOpen); setTenantDropOpen(false); }}
              >
                <Building2 size={13} />
                ...
                <div className="header-dropdown-menu">
                  {branches.map((b) => (
                    <button
                      ...
                      onClick={() => { switchBranch(b.id); setBranchDropOpen(false); }}
                    >
                      <div>
                        <div>{b.name}</div>
                        <div className="muted-row" style={{ margin: 0 }}>{b.location}</div>
                      </div>
                    </button>
                  ))}
                </div>
            ...
            {/* Connection */}
            <span className={`conn-pill ${isOnline ? "is-online" : "is-offline"}`}>
